package coda.glumbis.common.entities.ai.control;
...
public class SmoothFlyingMoveControl extends MoveControl {
   ...
   public SmoothFlyingMoveControl(Mob entity, int maxTurn, boolean hoversInPlace) {
      super(entity);
   ...
   public void tick() {
      if (this.operation == MoveControl.Operation.MOVE_TO) {
         ...
         if (Math.abs(d1) > (double)1.0E-5F || Math.abs(d4) > (double)1.0E-5F) {
            float f2 = -((float)(Mth.atan2(d1, d4) * (double)(180F / (float)Math.PI)));
            f2 = Mth.clamp(Mth.wrapDegrees(f2), (float)(-this.maxTurn), (float)this.maxTurn);
